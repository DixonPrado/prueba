...
if ($parameters->get('redirect_edit', 'site') === 'site')
{
	...
	$editUrl = JUri::base() . 'index.php?option=com_config&'.$configViewer.'&id=' . (int) $mod->id . '&Itemid=' . $itemid . $redirectUri;
	$target  = '_self';
}
...
if ($count)
{
	...
	JFactory::getDocument()->addScript(\T4\Helper\Path::findInTheme('js/system/frontediting.js', true));
}
