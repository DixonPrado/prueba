...
(function() {
	...
	window.RegularLabs.AdminForm = window.RegularLabs.AdminForm || {
		...
		loadAjaxFields: function() {
			document.querySelectorAll('textarea[data-rl-ajax]').forEach((el) => {
				...
				attributes = createCompressedAttributes(attributes);

				const url = `index.php?option=com_ajax&plugin=regularlabs&format=raw&fieldid=${el.id}&${attributes}`;
	...
	RegularLabs.AdminForm.loadAjaxFields();
	RegularLabs.AdminForm.removeEmptyControlGroups();
