...
class Document
{
	 ...
	 */
	public static function script($name, $attributes = ['defer' => true], $dependencies = [])
	{
		self::getAssetManager()->registerAndUseScript(
			$name,
			str_replace('.', '/', $name) . '.min.js',
			[],
			$attributes,
			$dependencies
		);
	 ...
	 * @param array  $attributes
	 */
	public static function style($name, $attributes = [])
	{
		self::getAssetManager()->registerAndUseStyle(
			$name,
			str_replace('.', '/', $name) . '.min.css',
			[],
			$attributes
		);
	}
